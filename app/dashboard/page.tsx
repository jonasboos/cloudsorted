  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="grid md:grid-cols-2 gap-5 sm:gap-8 mb-10 sm:mb-16">
        <div className="border-4 border-black p-5 sm:p-8 hover:bg-black hover:text-white transition-colors group">
          <div className="flex items-center justify-between gap-4 mb-8 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl font-black uppercase tracking-tighter">Google Drive</h2>
            <Cloud className="size-8 group-hover:text-white" />
          </div>
          <div className="flex items-center gap-2 mb-6 font-bold uppercase text-xs sm:text-sm">
            <CheckCircle2 className="size-5 text-green-500 group-hover:text-green-400" />
            <span className="min-w-0 break-all">Verbunden (demo@example.com)</span>
          </div>
          <Button className="w-full bg-black text-white hover:bg-white hover:text-black border-4 border-transparent hover:border-black rounded-none h-14 font-black uppercase tracking-widest text-xs sm:text-sm group-hover:bg-white group-hover:text-black group-hover:border-black group-hover:hover:bg-black group-hover:hover:text-white group-hover:hover:border-white">
        ...
        <div className="border-4 border-black p-5 sm:p-8 hover:bg-black hover:text-white transition-colors group">
          <div className="flex items-center justify-between gap-4 mb-8 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl font-black uppercase tracking-tighter">OneDrive</h2>
            <Cloud className="size-8 group-hover:text-white" />
          </div>
          <div className="flex items-center gap-2 mb-6 font-bold uppercase text-xs sm:text-sm">
          ...
          <Button className="w-full bg-white text-black hover:bg-black hover:text-white border-4 border-black rounded-none h-14 font-black uppercase tracking-widest text-xs sm:text-sm group-hover:bg-black group-hover:text-white group-hover:border-white group-hover:hover:bg-white group-hover:hover:text-black group-hover:hover:border-transparent">
            Verbinden
          </Button>
        </div>
      </div>

      <div className="border-4 border-black p-5 sm:p-8">
        <h2 className="text-2xl sm:text-3xl font-black uppercase tracking-tighter mb-6 sm:mb-8">Letzte Aktivitäten</h2>
        <div className="divide-y-4 divide-black border-t-4 border-black">
          {[
            ...
            <div key={index} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 py-5 sm:py-6">
              <div>
                <p className="font-black uppercase text-base sm:text-lg">{item.action}</p>
                <p className="font-bold uppercase text-xs text-gray-500">{item.time}</p>
              </div>
              <span className="w-fit bg-black text-white px-4 py-2 font-bold uppercase text-xs">
                {item.status}
              </span>
            </div>
          ))}
